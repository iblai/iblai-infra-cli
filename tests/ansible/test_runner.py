...
class TestBuildExtraVars:
    def test_includes_all_fields(self, project_state, setup_config):
        ...
        # Platform name defaults to "main"; Microsoft SSO defaults to disabled / empty
        assert extra["platform_name"] == "main"
        assert extra["microsoft_sso_enabled"] is False
        assert extra["microsoft_sso_client_id"] == ""
        assert extra["microsoft_sso_client_secret"] == ""
        assert extra["microsoft_sso_tenant_id"] == ""
        assert extra["microsoft_sso_organization"] == ""
